...
LINKEDIN_URL = '''
https://www.linkedin.com/jobs/search/?keywords=#&position=1&pageNum=0'''


@task(name="Obtener oferta laborales")
def get_offers(skill):
    url = requests.get(LINKEDIN_URL.replace('#', skill))

    if url.status_code == 200:
        print("EXTRACCIÓN DE DATA...")
        html = BeautifulSoup(url.text, 'html.parser')
        dataOfertas = html.find(
            'ul',
        ...
        for oferta in listaOfertas:
            ...
            urlOferta = oferta.find(
                'a',
                {'class': '''base-card__full-link absolute top-0 right-0
                bottom-0 left-0 p-0 z-[2]'''})
            dicOferta = {
